...
IMPLEMENT_SERVERCLASS_ST(CWeaponAlyxGun_s, DT_WeaponAlyxGun_s)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_alyxgun_s, CWeaponAlyxGun_s );
PRECACHE_WEAPON_REGISTER(weapon_alyxgun_s);

BEGIN_DATADESC( CWeaponAlyxGun_s )
END_DATADESC()

acttable_t	CWeaponAlyxGun_s::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponAlyxGun_s);
...
//=========================================================
CWeaponAlyxGun_s::CWeaponAlyxGun_s( )
{
	m_fMinRange1		= 1;
	m_fMaxRange1		= 5000;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAlyxGun_s::Precache( void )
{
	BaseClass::Precache();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponAlyxGun_s::Equip( CBaseCombatCharacter *pOwner )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponAlyxGun_s::WeaponRangeAttack1Condition( float flDot, float flDist )
{
#ifdef HL2_EPISODIC
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponAlyxGun_s::WeaponRangeAttack2Condition( float flDot, float flDist )
{
	return COND_NONE;
}
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponAlyxGun_s::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAlyxGun_s::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
//-----------------------------------------------------------------------------//test
void CWeaponAlyxGun_s::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
const Vector& CWeaponAlyxGun_s::GetBulletSpread( void )
{
	if (m_bIsIronsighted)
	{
...
//-----------------------------------------------------------------------------
float CWeaponAlyxGun_s::GetMinRestTime( void )
{
	//if ( IsAlyxInInjuredMode() )
	//	return 1.5f;
...
//-----------------------------------------------------------------------------
float CWeaponAlyxGun_s::GetMaxRestTime( void )
{
	//if ( IsAlyxInInjuredMode() )
	//	return 3.0f;
...
void CWeaponAlyxGun_s::PrimaryAttack(void)
{
	...
	CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), SOUNDENT_VOLUME_EMPTY, 0.2, pPlayer);
...
void CWeaponAlyxGun_s::HoldIronsight(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
...
bool CWeaponAlyxGun_s::Reload(void)
{
