...
class CWeaponSOS : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS(CWeaponSOS, CBaseHLCombatWeapon);
...
public:
	...
	{
		static Vector vitalAllyCone = VECTOR_CONE_15DEGREES; //בûכמ 10
...
IMPLEMENT_SERVERCLASS_ST(CWeaponSOS, DT_WeaponSOS)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_sos, CWeaponSOS);
PRECACHE_WEAPON_REGISTER(weapon_sos);

BEGIN_DATADESC(CWeaponSOS)
...
acttable_t	CWeaponSOS::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponSOS);

void CWeaponSOS::Precache(void)
{
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponSOS::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSOS::Operator_ForceNPCFire(CBaseCombatCharacter *pOperator, bool bSecondary)
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponSOS::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch (pEvent->event)
	{
...
//-----------------------------------------------------------------------------
float CWeaponSOS::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponSOS::GetMaxRestTime()
{
	if (hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE)
	{
		return 1.5f;
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponSOS::StartReload(void)
{
	...
	m_flNextPrimaryAttack = gpGlobals->curtime + SequenceDuration(); // +SequenceDuration();
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponSOS::FinishReload(void)
{
...
void CWeaponSOS::FillClip(void)
{
	...
	if ( pOwner->GetAmmoCount( m_iPrimaryAmmoType ) > 0 )
	{
		...
		else if (Clip1() < GetMaxClip1()) //וכסוטפ
		{
			m_iClip1++;
			m_iClip1++;
...
//
//-----------------------------------------------------------------------------
void CWeaponSOS::PrimaryAttack(void)
{
	...
	pPlayer->SetMuzzleFlashTime(gpGlobals->curtime + 0.1);
	...
	pPlayer->ViewPunch(QAngle(random->RandomFloat(-8, -8), random->RandomFloat(-8, 8), 0));
...
//
//-----------------------------------------------------------------------------
void CWeaponSOS::SecondaryAttack(void)
{
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
...
//-----------------------------------------------------------------------------
void CWeaponSOS::ItemPostFrame(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponSOS::ItemHolsterFrame(void)
{
	// Must be player held
...
void CWeaponSOS::SetSkin(int skinNum)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
//==================================================
/*
void CWeaponSOS::WeaponIdle( void )
{
//Only the player fires this way so we can cast
